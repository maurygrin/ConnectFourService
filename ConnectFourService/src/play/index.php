...
if ((strcasecmp($id, "") == 0)){
    ...
    if((strcasecmp($fil[2], "Random") == 0)){
     ...
     //   echo ("{response: true, ack_move: {slot: 3,isWin: false,isDraw: false,row: []},move: {slot: 4,isWin: false,isDraw: false,row: []}}");
        echo json_encode(array("response" => $response, "ack_move" => array("slot" => $column, "isWin" => $isWin, "isDraw" => $isDraw, "row" => array()), "move" => array("slot" => $xrand, "isWin" => $isWin, "isDraw" => $isDraw, "row" => array())));
